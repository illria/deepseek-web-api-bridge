...
# ---------------- V21 compatibility / hotfix helpers ----------------

KNOWN_SKILL_NAMES = [
    "opennews", "polymarket", "rootdata-crypto", "sim",
    "polyhub_account", "polyhub_copy", "polyhub_discover",
    "browser-automation", "weather", "session-logs", "python-debugpy",
    "node-connect", "taskflow", "notion", "healthcheck", "diagram-maker",
    "canvas", "meme-maker", "video-frames", "skill-creator",
]


def detect_skill_request(messages_or_text: list[OpenAIMessage] | str) -> dict[str, Any] | None:
    if isinstance(messages_or_text, str):
        raw = messages_or_text
    else:
        raw = unwrap_current_user_message(_last_user_content(messages_or_text))
    ...
    invoke_words = [
        "运行", "执行", "调用", "使用", "跑一下", "跑一跑", "看一下", "查看",
        "run", "execute", "invoke", "use", "call", "start",
    ]
    ...
    if not skill:
        m = re.search(r"\b(?:run|execute|invoke|use|call)\s+(?:skill\s+)?([a-zA-Z0-9_.-]{2,64})\b", low)
        if m:
            skill = m.group(1)
    ...
    invoked = any(w.lower() in low for w in invoke_words) or skill.lower() == low.strip()
    if not invoked:
        return None

    args = text
    for w in invoke_words:
        args = re.sub(re.escape(w), " ", args, flags=re.I)
    args = re.sub(re.escape(skill), " ", args, flags=re.I).strip()
    args = re.sub(r"\s+", " ", args).strip(" ：:，,。")

    return {"skill": skill, "args": args, "raw": text}
...
def _looks_like_any(name: str, needles: list[str]) -> bool:
    low = name.lower()
    return any(n.lower() in low for n in needles)


def _best_tool_by_names(tools: list[dict[str, Any]] | None, names: list[str]) -> tuple[dict[str, Any], str] | tuple[None, None]:
    for tool in tools or []:
        name = _tool_name(tool)
        if name and _looks_like_any(name, names):
            return tool, name
    return None, None


def _generic_skill_tool_call(skill_info: dict[str, Any], tools: list[dict[str, Any]]) -> list[dict[str, Any]]:
    skill = skill_info["skill"]
    args = skill_info.get("args") or ""
    raw = skill_info.get("raw") or skill

    for tool in tools:
        name = _tool_name(tool)
        if not name:
            continue
        if name.lower() == skill.lower() or name.lower().replace("_", "-") == skill.lower().replace("_", "-"):
            call_args = _make_tool_arguments(tool, {
                "skill": skill, "skillname": skill, "commandname": skill,
                "command": args or raw, "input": args or raw, "query": args or raw, "q": args or raw,
            })
            return [_openai_tool_call(name, call_args)]

    tool, name = _best_tool_by_names(tools, [
        "run_skill", "invoke_skill", "execute_skill", "skill_runner", "openclaw_skill",
        "slash_command", "command_dispatch", "dispatch_skill", "run_command_skill",
    ])
    if tool and name:
        command = f"/{skill} {args}".strip()
        call_args = _make_tool_arguments(tool, {
            "skill": skill, "skillname": skill, "commandname": skill,
            "command": command, "input": args or command, "args": args,
        })
        return [_openai_tool_call(name, call_args)]
...
def infer_skill_tool_call(messages: list[OpenAIMessage], tools: list[dict[str, Any]] | None) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
    skill_info = detect_skill_request(messages)
    if not skill_info:
        return [], None
    ...
    calls = _generic_skill_tool_call(skill_info, tools)
    if calls:
        return calls, skill_info

    skill = str(skill_info["skill"]).lower()
    args = skill_info.get("args") or ""
    raw = skill_info.get("raw") or skill

    web_first_skills = {"opennews", "polymarket", "rootdata-crypto", "sim", "weather"}
    browser_skills = {"browser-automation"}

    if skill in web_first_skills:
        query_map = {
            "opennews": args or "latest crypto finance encryption market news",
            "polymarket": args or "Polymarket latest prediction markets trending markets",
            "rootdata-crypto": args or "RootData crypto latest projects fundraising news",
            "sim": args or "Dune Sim API wallet token onchain query",
            "weather": args or raw,
        }
        query = query_map.get(skill, args or raw)
        tool, name = _best_tool_by_names(tools, ["web_search", "search", "browser_search", "google", "browse", "browser"])
        if tool and name:
            call_args = _make_tool_arguments(tool, {
                "query": query, "q": query, "input": query, "url": query, "command": query,
            })
            return [_openai_tool_call(name, call_args)], skill_info

    if skill in browser_skills:
        tool, name = _best_tool_by_names(tools, ["browser", "browse", "browser_automation"])
        if tool and name:
            call_args = _make_tool_arguments(tool, {"input": args or raw, "query": args or raw, "command": args or raw})
            return [_openai_tool_call(name, call_args)], skill_info

    tool, name = _best_tool_by_names(tools, [
        "terminal", "shell", "bash", "execute_command", "run_command", "command",
        "exec", "run_shell", "local_shell",
    ])
    if tool and name:
        safe_skill = re.sub(r"[^a-zA-Z0-9_.-]", "", skill)
        cmd = (
            "python3 - <<'PY'\n"
            "from pathlib import Path\n"
            f"skill={safe_skill!r}\n"
            "roots=[Path.cwd()/'skills', Path.cwd()/'.agents'/'skills', Path.home()/'.agents'/'skills', Path.home()/'.openclaw'/'skills']\n"
            "found=[]\n"
            "for root in roots:\n"
            "    if not root.exists():\n"
            "        continue\n"
            "    for p in root.rglob('SKILL.md'):\n"
            "        if p.parent.name==skill or skill in str(p.parent):\n"
            "            found.append(p)\n"
            "if not found:\n"
            "    print(f'Skill {skill} not found in common roots. If OpenClaw has a command-dispatch tool, expose it to the model provider.')\n"
            "else:\n"
            "    for p in found[:3]:\n"
            "        print('---', p)\n"
        ...
        call_args = _make_tool_arguments(tool, {"command": cmd, "cmd": cmd, "input": cmd})
        return [_openai_tool_call(name, call_args)], skill_info
...
def _flatten_json_text(value: Any, max_chars: int = 6000) -> str:
    if value is None:
        return ""
        ...
        for item in content:
            if isinstance(item, dict):
                if item.get("type") == "text":
                    ...
                    parts.append(_tool_content_to_text(item.get("content")))
                else:
                    parts.append(_flatten_json_text(item))
            else:
                parts.append(str(item))
        return "\n".join(p for p in parts if p.strip()).strip()
    return _flatten_json_text(content)


def _summarize_search_like_text(text: str, max_items: int = 8) -> str:
    text = (text or "").strip()
    if not text:
        return ""
    try:
    ...
    if isinstance(obj, dict):
        for key in ["results", "items", "data", "documents", "news", "organic"]:
            if isinstance(obj.get(key), list):
                items = obj[key]
                break
        if not items and any(k in obj for k in ["title", "url", "snippet", "content"]):
            items = [obj]
    ...
    if items:
        ...
        for i, item in enumerate(items[:max_items], 1):
            if isinstance(item, dict):
                title = str(item.get("title") or item.get("name") or item.get("headline") or f"结果 {i}").strip()
...
def fast_finalize_tool_results(messages: list[OpenAIMessage], max_chars: int = 6000) -> str:
    tool_blocks = []
    for msg in messages:
        if msg.role != "tool":
            continue
        name = (msg.name or msg.tool_call_id or "tool").strip()
        text = _tool_content_to_text(msg.content)
        text = _summarize_search_like_text(text)
        text = _clip_text(text, max_chars)
        if text:
            tool_blocks.append((name, text))

    if not tool_blocks:
        return ""

    if len(tool_blocks) == 1:
        name, text = tool_blocks[0]
        return f"工具 `{name}` 已返回结果：\n\n{text}"

    parts = ["工具执行结果如下："]
    for idx, (name, text) in enumerate(tool_blocks, 1):
        parts.append(f"\n{idx}. `{name}`\n{text}")
    return "\n\n".join(parts).strip()


def should_fast_finalize_tool_results(messages: list[OpenAIMessage]) -> bool:
    return any(m.role == "tool" for m in messages)
